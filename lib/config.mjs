...
/**
 * BakerConfig the configuration file useFlowDir
 *
 ...
 * @property {Boolean=} useFlowDir - enables usage of internal folders separation inside `src/forms` per flow
...
export function resolveSourceConfig ({ filepath, config }) {
  ...
  const projectFolder = config.useFlowDir ? paths[3] : paths[2]
  const flowFolder = config.useFlowDir ? paths[2] : null
  const srcPath = flowFolder ? `${flowFolder}\\${projectFolder}` : projectFolder
  const isFileFromFormsDir = paths[1] === FORMS_FOLDER

  // when the modified file doesn't match `FORMS_FOLDER`
  // falls back to rebuild every file with the same extension
  if (!isFileFromFormsDir) {
    const allBundles = getAllBundles({ fileExtension: ext, entryPoints: config.entryPoints })
    return allBundles
  }

  return [{
    entry: `src\\${FORMS_FOLDER}\\${srcPath}\\${config.entryPoints[ext.replace('.', '')]}`,
