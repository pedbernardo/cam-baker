 ...
 * @property {("info"|"error"|"warn")} level - determines the logging level (default: info)
 ...
 * @property {Boolean=} printStackError - logs the stacktrace error when logging erros (default: true)
 */
...
export function printCamundaRun ({ camundaRun, usePrefix, port }) {
  ...
  // @todo const runAutoMigration = colors.grey(`| auto-migration: ${(camundaRun?.autoMigration ? colors.bold.green('on') : colors.bold.red('off'))}`)
  ...
  return {
    ...
    stop: text => spinner.stopAndPersist({
      ...
      text: ' ',
      prefixText: `${runTitle} ${runHost} ${runVersion} ${runAutoDeploy}`
      // @todo prefixText: `${runTitle} ${runHost} ${runVersion} ${runAutoDeploy} ${runAutoMigration}`
    }),
